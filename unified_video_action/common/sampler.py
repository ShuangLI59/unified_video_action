...
class SequenceSamplerUMIMy:
    def __init__(self,
        ...
        if sampler_name=='SequenceSamplerUMIMyfewdata':
            if num_data==200:
                json_file_path = '/'.join(dataset_path.split('/')[:-1]) + '/sampled_200index.json'
            elif num_data==500:
                json_file_path = '/'.join(dataset_path.split('/')[:-1]) + '/sampled_500index.json'
            else:
                raise NotImplementedError
        ...
        for i in range(len(episode_ends)):
            ...
            for current_idx in range(start_idx, stop_idx):
                ...
                if not action_padding:
                    num_valid = min(key_horizon['action'], (current_idx - start_idx) // key_down_sample_steps['action'] + 1)
    ...
    def sample_sequence(self, idx):
        ...
        if current_idx > end_idx-1:
            ...
            if self.has_action_mocap:
                ...
                output_mocap = input_arr_mocap[slice_start: end_idx: action_down_sample_steps]
        else:
            ...
            if not self.action_padding:
                assert output.shape[0] == action_horizon or output.shape[0] > action_horizon//2
