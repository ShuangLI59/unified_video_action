...
class SequenceSamplerUMI:
    def __init__(
        ...
        for i in range(len(episode_ends)):
            ...
            for current_idx in range(start_idx, end_idx):
                if (
                    ...
                    and end_idx
                    < current_idx
                    + (key_horizon["action"] - 1) * key_down_sample_steps["action"]
                    + 1
                ):
                    continue
        ...
        self.ignore_rgb_is_applied = (
            False  # speed up the interation when getting normalizaer
        )
    ...
    def sample_sequence(self, idx):
        ...
        for key in obs_keys:
            ...
            if key in self.rgb_keys:
                ...
                output = input_arr[
                    slice_start : current_idx + 1 : this_downsample_steps
                ]  # (32, 224, 224, 3)
                ...
                if output.shape[0] < this_horizon:
                    padding = np.repeat(
                        output[:1], this_horizon - output.shape[0], axis=0
                    )
        ...
        slice_end = min(
            end_idx, current_idx + (action_horizon - 1) * action_down_sample_steps + 1
        )
...
class SequenceSamplerUMIMy:
    def __init__(
        ...
        if sampler_name == "SequenceSamplerUMIMyfewdata":
            if num_data == 200:
                json_file_path = (
                    "/".join(dataset_path.split("/")[:-1]) + "/sampled_200index.json"
                )
            elif num_data == 500:
                json_file_path = (
                    "/".join(dataset_path.split("/")[:-1]) + "/sampled_500index.json"
                )
            else:
                raise NotImplementedError
            ...
            print("sample fewer data", num_data)
            ...
            if os.path.exists(json_file_path):
                ...
                print("load sampled_indices from json file", json_file_path)
                print(
                    "-------------------------------------------------------------------------------"
                )
                with open(json_file_path, "r") as json_file:
                    sampled_indices = json.load(json_file)
            else:
                print(
                    "-------------------------------------------------------------------------------"
                )
                print("save sampled_indices to json file", json_file_path)
                print(
                    "-------------------------------------------------------------------------------"
                )
                sampled_indices = random.sample(range(len(episode_ends)), num_data)
                sampled_indices.sort()

                with open(json_file_path, "w") as json_file:
                    json.dump(sampled_indices, json_file)
        ...
        for i in range(len(episode_ends)):
            ...
            if "towel" in dataset_path or "mouse" in dataset_path:
                stop_idx = end_idx
                # print('-------------------------------------------------------------------------------')
                # print(dataset_path, 'stop_idx = end_idx')
                # print('-------------------------------------------------------------------------------')
            else:
                stop_idx = end_idx + key_horizon["action"]
                # print('-------------------------------------------------------------------------------')
                # print(dataset_path, 'stop_idx = end_idx+key_horizon_action')
                # print('-------------------------------------------------------------------------------')

            # for current_idx in range(start_idx, end_idx):
            # for current_idx in range(start_idx, end_idx+key_horizon['action']):
            for current_idx in range(start_idx, stop_idx):
                ## remove this
                ...
                if not action_padding:
                    num_valid = min(
                        key_horizon["action"],
                        (current_idx - start_idx) // key_down_sample_steps["action"]
                        + 1,
                    )
                    if num_valid <= key_horizon["action"] // 2:
                        continue
                ################################################################################################################

                if current_idx >= end_idx:
                    before_first_grasp = before_first_grasp
                else:
                    if gripper_width[current_idx] < gripper_width_threshold:
                        before_first_grasp = False
                indices.append((current_idx, start_idx, end_idx, before_first_grasp))
        ...
        if "action" in replay_buffer:
            ...
            if len(actions_mocap) > 0:
                self.replay_buffer["action_mocap"] = np.concatenate(
                    actions_mocap, axis=-1
                )
        ...
        # for idx in range(820, 850):
        #     self.sample_sequence(idx)
    ...
    def sample_sequence(self, idx):
        ...
        for key in obs_keys:
            ...
            if key in self.rgb_keys:
                ...
                assert slice_start < end_idx
                # print(f'---------------------------- {key} -----------------------------')
                # print(slice_start, current_idx + 1, this_downsample_steps)

                if current_idx > end_idx - 1:
                    output = input_arr[slice_start:end_idx:this_downsample_steps]
                    assert output.shape[0] < num_valid
        ...
        num_valid = min(
            action_horizon, (current_idx - start_idx) // action_down_sample_steps + 1
        )
        slice_start = current_idx - (num_valid - 1) * action_down_sample_steps

        if current_idx > end_idx - 1:
            output = input_arr[slice_start:end_idx:action_down_sample_steps]

            if self.has_action_mocap:
                input_arr_mocap = self.replay_buffer["action_mocap"]
                output_mocap = input_arr_mocap[
                    slice_start:end_idx:action_down_sample_steps
                ]
        else:
            ...
            if self.has_action_mocap:
                input_arr_mocap = self.replay_buffer["action_mocap"]
                output_mocap = input_arr_mocap[
                    slice_start : current_idx + 1 : action_down_sample_steps
                ]

            if not self.action_padding:
                assert (
                    output.shape[0] == action_horizon
                    or output.shape[0] > action_horizon // 2
                )
        ...
        if output.shape[0] < action_horizon:
            if current_idx > end_idx - 1:
                padding = np.repeat(
                    output[-1:], action_horizon - output.shape[0], axis=0
                )
                output = np.concatenate([output, padding], axis=0)

                if self.has_action_mocap:
                    padding_mocap = np.repeat(
                        output_mocap[-1:],
                        action_horizon - output_mocap.shape[0],
                        axis=0,
                    )
                    output_mocap = np.concatenate([output_mocap, padding_mocap], axis=0)
            else:
                padding = np.repeat(
                    output[:1], action_horizon - output.shape[0], axis=0
                )
                ...
                if self.has_action_mocap:
                    padding_mocap = np.repeat(
                        output_mocap[:1], action_horizon - output_mocap.shape[0], axis=0
                    )
