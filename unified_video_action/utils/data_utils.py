...
def resize_image(cfg, x):
    if cfg.name == 'uva':
        ...
        if 'libero' in cfg.task.name:
            B, T, C, H, W = x['obs']['agentview_rgb'].shape # [1, 32, 3, 180, 320]
            resized_tensor = F.interpolate(x['obs']['agentview_rgb'].float().view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
            x['obs']['image'] = resized_tensor.view(B, T, C, resize, resize) # [1, 32, 3, 180, 180]
            del x['obs']['agentview_rgb']
            
        elif 'umi' in cfg.task.name:
            B, T, C, H, W = x['obs']['camera0_rgb'].shape # [1, 32, 3, 180, 320]
            resized_tensor = F.interpolate(x['obs']['camera0_rgb'].contiguous().float().view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
            x['obs']['image'] = resized_tensor.view(B, T, C, resize, resize) # [1, 32, 3, 180, 180]
            del x['obs']['camera0_rgb']
            
        elif 'toolhang' in cfg.task.name:
            B, T, C, H, W = x['obs']['sideview_image'].shape # [1, 32, 3, 180, 320]
            resized_tensor = F.interpolate(x['obs']['sideview_image'].contiguous().float().view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
            x['obs']['image'] = resized_tensor.view(B, T, C, resize, resize) # [1, 32, 3, 180, 180]
            del x['obs']['sideview_image']    
        
            resized_tensor = F.interpolate(x['obs']['robot0_eye_in_hand_image'].contiguous().float().view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
            ...
            if resize != H:
                resized_tensor = F.interpolate(x['obs']['image'].contiguous().view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
                x['obs']['image'] = resized_tensor.view(B, T, C, resize, resize)
    else:
        if 'libero' in cfg.task.name:
            x['obs']['image'] = x['obs']['agentview_rgb']
            B, T, C, H, W = x['obs']['image'].shape
            del x['obs']['agentview_rgb']
            ...
            x['obs']['image'] = x['obs']['sideview_image']
            B, T, C, H, W = x['obs']['image'].shape
            del x['obs']['sideview_image']
...
def resize_image_eval(task_name, obs_dict):
    if 'libero' in task_name:
        ...
        if 'camera0_rgb' in obs_dict:
            resize = 256
            B, T, C, H, W = obs_dict['camera0_rgb'].shape
            assert H==resize 
            assert W==resize
    ...
    resize = 256
    if H != resize:
        resized_tensor = F.interpolate(obs_dict['image'].view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
        obs_dict['image'] = resized_tensor.view(B, T, C, resize, resize)
    
        if 'wrist_image' in obs_dict:
            resized_tensor = F.interpolate(obs_dict['wrist_image'].view(B*T, C, H, W), size=(resize, resize), mode='bilinear', align_corners=False)
...
def decode_from_sample(vae_model, z):
    with torch.no_grad():
        ...
        # pred = vae_model.module.model.decode_from_sample(z)
    return pred
...
def select_frames(x, T, eval=False, select_timesteps=4, different_history_freq=False, flag_different_freq_and_interval=False):
    if different_history_freq:
        if eval:
            indices = torch.arange(0, T, step=T//select_timesteps) + select_timesteps - 1
        else:
            indices = torch.arange(0, T, step=T//(select_timesteps*2)) + select_timesteps - 1
            
            if flag_different_freq_and_interval:
                indices = torch.cat([torch.tensor(random.choice(combinations)), indices[indices.shape[0]//2:]])
            else:
                indices = torch.cat([torch.tensor(random.choice(different_freq_T16)), indices[indices.shape[0]//2:]])
...
def process_data(batch, task_name='', eval=False, **kwargs):
    ...
    if ('umi' in task_name):
        ...
        x, indices = select_frames(x, T, eval=eval, different_history_freq=kwargs['different_history_freq'], 
                                   flag_different_freq_and_interval=kwargs['flag_different_freq_and_interval'])
    ...
    if kwargs['use_proprioception']:
        if 'toolhang' in task_name:
            ...
            if train:
                ...
                robot0_eef_pos, robot0_eef_pos_pred = torch.chunk(batch['obs']['robot0_eef_pos'], 2, dim=1)
                robot0_eef_quat, robot0_eef_quat_pred = torch.chunk(batch['obs']['robot0_eef_quat'], 2, dim=1)
                robot0_gripper_qpos, robot0_gripper_qpos_pred = torch.chunk(batch['obs']['robot0_gripper_qpos'], 2, dim=1)
            else:
            ...
            if kwargs['different_history_freq']:
                if train:
                    ...
                    robot0_eef_quat = robot0_eef_quat[:, indices[:indices.shape[0]//2]]
                    robot0_gripper_qpos = robot0_gripper_qpos[:, indices[:indices.shape[0]//2]]
                else:
            ...
            proprioception_input = {'robot0_eef_pos': robot0_eef_pos, 
            ...
            if train:
                robot0_eef_pos, robot0_eef_pos_pred = torch.chunk(batch['obs']['robot0_eef_pos'], 2, dim=1)
                robot0_eef_rot_axis_angle, robot0_eef_rot_axis_angle_pred = torch.chunk(batch['obs']['robot0_eef_rot_axis_angle'], 2, dim=1)
                robot0_gripper_width, robot0_gripper_width_pred = torch.chunk(batch['obs']['robot0_gripper_width'], 2, dim=1)
                robot0_eef_rot_axis_angle_wrt_start, robot0_eef_rot_axis_angle_wrt_start_pred = torch.chunk(batch['obs']['robot0_eef_rot_axis_angle_wrt_start'], 2, dim=1)
            else:
                robot0_eef_pos = batch['obs']['robot0_eef_pos']
                robot0_eef_rot_axis_angle = batch['obs']['robot0_eef_rot_axis_angle']
                robot0_gripper_width = batch['obs']['robot0_gripper_width']
                robot0_eef_rot_axis_angle_wrt_start = batch['obs']['robot0_eef_rot_axis_angle_wrt_start']
                robot0_eef_pos_pred = None
            ...
            if kwargs['different_history_freq']:
                if len(indices.shape) == 1:
                    ...
                    if train:
                        robot0_eef_pos = robot0_eef_pos[:, indices[:indices.shape[0]//2]]
                        robot0_eef_rot_axis_angle = robot0_eef_rot_axis_angle[:, indices[:indices.shape[0]//2]]
                        robot0_gripper_width = robot0_gripper_width[:, indices[:indices.shape[0]//2]]
                        robot0_eef_rot_axis_angle_wrt_start = robot0_eef_rot_axis_angle_wrt_start[:, indices[:indices.shape[0]//2]]
                    else:
                        robot0_eef_pos = robot0_eef_pos[:, indices]
                        robot0_eef_rot_axis_angle = robot0_eef_rot_axis_angle[:, indices]
                        robot0_gripper_width = robot0_gripper_width[:, indices]
                        robot0_eef_rot_axis_angle_wrt_start = robot0_eef_rot_axis_angle_wrt_start[:, indices]
                else:
                    # Each data point in the batch has a different set of indices
                    length = indices.shape[1] # [bs, 8]
                    if train: 
                        length = length // 2 # Only use the first half of the indices for training

                    # Create index tensors for batched indexing
                    batch_indices = torch.arange(indices.shape[0], device=indices.device).unsqueeze(-1).expand(-1, length)

                    # Use advanced indexing to directly gather the required elements
                    robot0_eef_pos = robot0_eef_pos[batch_indices, indices[:, :length]]
                    robot0_eef_rot_axis_angle = robot0_eef_rot_axis_angle[batch_indices, indices[:, :length]]
                    robot0_gripper_width = robot0_gripper_width[batch_indices, indices[:, :length]]
                    robot0_eef_rot_axis_angle_wrt_start = robot0_eef_rot_axis_angle_wrt_start[batch_indices, indices[:, :length]]
...
def get_vae_latent(x, vae_model, eval=False, proprioception_input={}):
    ...
    if proprioception_input is not None:
        if 'second_image' in proprioception_input:
            second_image_z, _ = extract_latent_autoregressive(vae_model, proprioception_input['second_image'])
            proprioception_input['second_image_z'] = second_image_z
        if 'pred_second_image' in proprioception_input:
            pred_second_image_z, _ = extract_latent_autoregressive(vae_model, proprioception_input['pred_second_image'])
