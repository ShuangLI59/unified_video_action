...
class TrainUnifiedVideoActionWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if "deepspeed_config" in cfg.training and cfg.training.deepspeed_config is not None:
            language_emb_model = None # HACK: When training umi dataset on multiple nodes
        self.model: UnifiedVideoActionPolicy = hydra.utils.instantiate(cfg.model.policy,
    ...
    def run(self):
        ...
        if "deepspeed_config" in cfg.training and cfg.training.deepspeed_config is not None:
            ...
            accelerator = Accelerator(log_with='wandb', mixed_precision=self.cfg.training.mixed_precision, deepspeed_plugin=deepspeed_plugin, kwargs_handlers=[ddp_kwargs])
        else:   
            accelerator = Accelerator(log_with='wandb', mixed_precision=self.cfg.training.mixed_precision)
        ...
        if "deepspeed_config" not in cfg.training or cfg.training.deepspeed_config is None:
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec) as tepoch:
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    # compute loss
                    if "deepspeed_config" in cfg.training and cfg.training.deepspeed_config is not None: # HACK: use autocast for bf16 training
            ...
            if cfg.model.policy.autoregressive_model_params.predict_video:
                fvd_log = test_video_fvd(cfg, policy, val_dataloader, local_epoch_idx, self.output_dir, device)
                step_log.update(fvd_log)
                
                
            # ========= evaluate val action error =========
            if cfg.model.policy.action_model_params.predict_action and 'env_runner' not in cfg.task:
                ## if has similartor, skip this
                act_log = test_action_l2(cfg, policy, val_dataloader, local_epoch_idx, self.output_dir, device)
